  ...
  if (address < PHYS_BASE)
    {
      ...
      /* Find existing page. */
      p.virtual_address = (void *) pg_round_down (address);
      e = hash_find (thread_current ()->vm, &p.hash_elem);
      if (e != NULL)
      ...
      */
      if ((p.virtual_address > PHYS_BASE - STACK_MAX) && ((void *)thread_current()->user_esp - 32 < address))
      {
        return page_allocate (p.virtual_address, false);
      }
    }
...
static bool do_page_in (struct page *p) {
  ...
  p->page_frame = frame_alloc_and_lock (p);
  ...
  if (p->swap_sector != (block_sector_t) -1)
    ...
    {
      /* Get data from file. */
      off_t read_bytes = file_read_at (p->file, p->page_frame->base,
                                        p->file_bytes, p->file_offset);
      off_t zero_bytes = PGSIZE - read_bytes;
      memset (p->page_frame->base + read_bytes, 0, zero_bytes);
  ...
  else
    {
      ...
      memset (p->page_frame->base, 0, PGSIZE);
    }
...
bool page_in (void *fault_addr) {
  ...
  ASSERT (lock_held_by_current_thread (&p->page_frame->lock));

  /* Install frame into page table. */
  success = pagedir_set_page (thread_current ()->pagedir, p->virtual_address,
                              p->page_frame->base, !p->read_only);

  /* Release frame. */
  frame_unlock (p->page_frame);
...
bool page_out (struct page *p)
{
  ...
  /* Mark page not present in page table, forcing accesses by the
     ...
     page. */
  pagedir_clear_page(p->thread->pagedir, (void *) p->virtual_address);

  /* Has the frame been modified? */
  /* If the frame has been modified, set 'dirty' to true. */
  dirty = pagedir_is_dirty (p->thread->pagedir, (const void *) p->virtual_address);
  ...
  else
  {
    if (dirty)
    {
      ...
      {
        ok = file_write_at(p->file, (const void *) p->page_frame->base, p->file_bytes, p->file_offset);
      }
    }
  }
...
bool page_accessed_recently (struct page *p) {
    ...
    was_accessed = pagedir_is_accessed (p->thread->pagedir, p->virtual_address);
    if (was_accessed) {
        pagedir_set_accessed (p->thread->pagedir, p->virtual_address, false);
    }
    return was_accessed;
}
...
struct page* page_allocate (void *vaddr, bool read_only) {
  ...
  if (p != NULL)
    {
      p->virtual_address = pg_round_down (vaddr);
      ...
      p->swap_sector = (block_sector_t) -1;
      ...
      if (hash_insert (t->vm, &p->hash_elem) != NULL)
        {
          /* Already mapped. */
          free (p);
          p = NULL;
...
void page_deallocate (void *vaddr) {
  ...
  hash_delete (thread_current ()->vm, &p->hash_elem);
  free (p);
}
...
unsigned page_hash (const struct hash_elem *e, void *aux UNUSED) {
  ...
  return ((uintptr_t) p->virtual_address) >> PGBITS;
}
...
bool page_lock (const void *addr, bool will_write) {
  ...
  if (p->page_frame == NULL)
    return (do_page_in (p)
            && pagedir_set_page (thread_current ()->pagedir, p->virtual_address,
                                 p->page_frame->base, !p->read_only));
  else
    return true;
}
